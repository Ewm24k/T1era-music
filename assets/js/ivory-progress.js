// =========================================================
// T1ERA MUSIC - IVORY-4 PROGRESS CHECKLIST MODULE
// Handles the visual progress terminal + Firestore status
// listening for the Ivory-4 model (Stage 0 - Stage 4 only).
// Stage 5 (Piano Arrangement & Styling) and Stage 6 (Adaptive
// Musical Quantization) are intentionally skipped for this
// model to preserve the authentic, unstyled solo performance.
// =========================================================
...
export function createIvoryProgressCardMarkup() {
  return `
    <div class="proc-card">
      <div class="proc-header">
        <h3 class="proc-title">T1ERA Ivory-4 Engine</h3>
...
export function setIvoryStepStatus(stepId, state) {
...
export function showIvoryTerminalFailure(message) {
...
/**
 * Watches Firestore for job status updates and drives the Ivory-4
 * progress checklist (Stage 0 - Stage 4, no styling, no quantization).
 * Falls back to a simulated progress timeline if Firestore snapshot
 * listening is blocked (mirrors the Nexus-6 / Vortex-Ultra fallback).
 */
export function openIvoryLiveTerminalConsole(userId, jobId) {
  ...
  if (!hasCard) { verificationTerminal.innerHTML = createIvoryProgressCardMarkup(); }

  setIvoryStepStatus("step-init", "success");
  setIvoryStepStatus("step-download", "success");
  setIvoryStepStatus("step-details", "loading");
  ...
  const handleIvoryMidiComplete = (midiUrl) => {
    setIvoryStepStatus("step-transcribe", "success");
    setIvoryStepStatus("step-repair", "success");
    setIvoryStepStatus("step-reconstruct", "success");
    setIvoryStepStatus("step-stabilize", "success");
    setIvoryStepStatus("step-complete", "success");
  ...
  const fallbackTimeout = setTimeout(() => {
    if (isFirestoreActive) {
      ...
      console.warn("[FIRESTORE BYPASS - IVORY] Active snapshot listening blocked. Switching to fallback...");

      if (!detailsSynced) {
        detailsSynced = true;
        setIvoryStepStatus("step-details", "success");
      }
      ...
      const fakeInterval = setInterval(() => {
        ...
        if (fakeProgress >= 40 && fakeProgress < 65) {
          setIvoryStepStatus("step-transcribe", "success");
          setIvoryStepStatus("step-repair", "loading");
        } else if (fakeProgress >= 65 && fakeProgress < 90) {
          setIvoryStepStatus("step-repair", "success");
          setIvoryStepStatus("step-reconstruct", "loading");
          setIvoryStepStatus("step-stabilize", "loading");
        } else if (fakeProgress >= 90) {
          clearInterval(fakeInterval);
          setIvoryStepStatus("step-reconstruct", "success");
          setIvoryStepStatus("step-stabilize", "success");
          handleIvoryMidiComplete(constructedMidiUrl);
        }
      }, 750);
    }
  }, 4000);
  ...
  if (db && userId !== "guest_studio_creator") {
    ...
    const unsubscribe = onSnapshot(jobRef, (snapshot) => {
      ...
      if (status === "QUEUED") {
        ...
        setIvoryStepStatus("step-transcribe", "loading");
        ...
        setIvoryStepStatus("step-reconstruct", "success");
        setIvoryStepStatus("step-stabilize", "success");
        setIvoryStepStatus("step-complete", "loading");
      } else if (status === "COMPLETED") {
        unsubscribe();
        handleIvoryMidiComplete(data.midiUrl);
      } else if (status === "FAILED") {
        unsubscribe();
        showIvoryTerminalFailure(data.error || "Unknown pipeline error.");
      }
    }, (error) => {
      console.warn("[FIRESTORE WARNING - IVORY] Active snapshot listening blocked.", error);
    });
  }
